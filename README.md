...
Скрипт поднимает схему во встроенном SQLite, грузит те же данные и прогоняет сложные запросы
и негативные проверки ограничений:

```bash
py verify.py     # ожидаемый финал: "РЕЗУЛЬТАТ: ВСЁ ПРОШЛО"
```

> **Порядок и сброс.** Всегда `schema.sql` → `seed.sql` → `triggers.sql`.
> `schema.sql` дропает таблицы вместе с триггерами, поэтому полный сброс — тот же порядок.
> Даты в `seed.sql` заданы относительно `CURRENT_DATE`, так что отчёты «за прошлый месяц»
> и «за неделю» наполнены при любой дате запуска.
...
БД-часть группового проекта (3 участника). Полный отчёт включает требования, формализацию
зависимостей, пошаговую нормализацию, анализ аномалий, транзакции и приложение по работе с AI.
Данный репозиторий — инженерная (исполняемая) часть проекта.
